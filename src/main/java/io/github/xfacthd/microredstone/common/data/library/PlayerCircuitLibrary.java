...
final class PlayerCircuitLibrary {
    ...
    @Nullable CircuitLibraryEntry addOrModifyEntry(CircuitLibraryEntry entry) {
    ...
    @Nullable UUID removeEntry(String name) {
    ...
    List<CircuitLibraryEntry> packEntries(Collection<PlayerCircuitLibrary> libraries) {
        ...
        if (ownedById.isEmpty() && foreignEntries.isEmpty()) {
            return List.of();
        }
