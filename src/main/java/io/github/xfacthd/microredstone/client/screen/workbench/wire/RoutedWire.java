...
public record RoutedWire(Wire wire, List<Section> sections) {
    ...
    public <T extends WireNode> @Nullable T findNode(NodePos pos, Class<T> type, Predicate<T> filter) {
        for (WireNode node : wire.getNodes()) {
    ...
    public @Nullable Section removeSection(NodePos posOne, NodePos posTwo) {
        for (int i = 0; i < sections.size(); i++) {
            Section section = sections.get(i);
    ...
    public @Nullable Section findIntersectedSection(NodePos pos) {
        for (Section section : sections) {
            if (section.intersects(pos)) {
                return section;
            }
