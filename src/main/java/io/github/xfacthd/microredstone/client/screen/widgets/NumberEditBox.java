    ...
    private static ValidatingEditBox.Validator toValidator(ParserConfig config)
    {
        return new ValidatingEditBox.Validator()
        {
            ...
            public TriState validate(String value)
            {
                ...
                if (matchValue.isEmpty()) return TriState.DEFAULT;
                if (!format.pattern.matcher(matchValue).matches()) return TriState.FALSE;
                if (config.getMaxLength(format) < matchValue.length()) return TriState.FALSE;
